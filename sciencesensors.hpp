...
{
    Q_OBJECT
    Q_PROPERTY(qreal decay_coeff READ decay_coeff WRITE setDelayCoeff NOTIFY delayCoeffChanged)
    Q_PROPERTY(qreal half_life READ half_life WRITE setHalfLife NOTIFY halfLifeChanged)
    Q_PROPERTY(qreal total_energy READ total_energy WRITE setTotalEnergy NOTIFY totalEnergyChanged)
    Q_PROPERTY(qreal uncertainty READ uncertainty WRITE setUncertainty NOTIFY uncertaintyChanged)
    Q_PROPERTY(qreal ozone_ppm READ ozone_ppm WRITE setOzonePPM NOTIFY ozonePPMChanged)
    Q_PROPERTY(qreal h2_ppm READ h2_ppm WRITE setH2PPM NOTIFY h2PPMChanged)
    Q_PROPERTY(
        qreal atomic_radius READ atomic_radius WRITE setAtomicRadius NOTIFY atomicRadiusChanged)
    Q_PROPERTY(
        qreal radioactivity READ radioactivity WRITE setRadioactivity NOTIFY radioactivityChanged)
